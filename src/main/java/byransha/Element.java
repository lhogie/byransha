...
import byransha.action.MethodAction;
import byransha.action.base.CopyIDToClipboard;
import byransha.action.base.Delete;
import byransha.action.base.Export;
import byransha.action.base.Export.CSVData;
import byransha.action.base.FreezingAction;
import byransha.action.base.JumpToAnotherNode;
import byransha.action.base.OpenInNewChat;
import byransha.action.base.ShowClassNode;
...
//		System.out.println(nbInstances + " creating " + getClass());
	...
	public final List<ProblemInElement> errors() {
		var errs = new ArrayList<ProblemInElement>();
		fillErrors(errs);
		return errs;
	}
	...
	private void fillLine(WrapPanel currentLine, Member m, ChatSheet sheet, Element out, int left) {
		...
		if (m instanceof Field field) {
			...
			replace.addActionListener(e -> {
				...
				new Chat(hub().currentUser()).append(list);
			});
	...
	public JComponent createBall(int diameter, int border, Chat chat) {
