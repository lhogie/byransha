...
import byransha.ui.swing.ComponentTable;
...
public class ListNodeView2<T extends BNode> extends NodeView<ListNode<T>> {

	private TranslatableTextArea label;

	public ListNodeView2(BGraph g, ListNode<T> l) {
		super(g, l);
	}
	...
	public ObjectNode describeAsJSON() {
		...
		viewedNode.elements.forEach(e -> a.add(e.describeAsJSON()));
	...
	public void writeTo(Sheet pane) {
		this.label = new TranslatableTextArea(g.translator);
		pane.appendToCurrentLine(label);
		pane.newLine();
		var t = new ComponentTable();
		var jlist = new JList();
		...
		jlist.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					if (index >= 0) {
						((ChatSheet) pane).chat.append((BNode) jlist.getModel().getElementAt(index));
					}
				}
			}
		});
		...
		pane.appendToCurrentLine(Utils.resizableScrollPane(jlist));
		updateLabel();
