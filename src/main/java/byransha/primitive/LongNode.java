...
public class LongNode extends PrimitiveValueNode<Long> {
	...
	public LongNode(Element parent, ID id) {
		this(parent, id, 0);
	}

	public LongNode(Element parent, ID id, long value) {
		super(parent, id);
		set(value);
	}
	...
	public void writeKishanView(ChatSheet sheet) {
		...
		addValueChangeListener((n, old, newValue) -> {
		...
		if (bounds != null) {
			...
			slider.setEnabled(!userEditable);
