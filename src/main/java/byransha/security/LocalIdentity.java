...
public class LocalIdentity {
	...
	/**
	 * Loads the existing X25519 network routing keys from disk. 
	 * If they do not exist, generates a new KeyPair and securely saves them.
	 *
	 * @return A valid KeyPair containing the node's local X25519 keys.
	 */
	public static KeyPair loadOrGenerateRoutingKeys() throws IOException, NoSuchAlgorithmException, InvalidKeySpecException {
		File securityDir = new File(Byransha.homeDirectory, SECURITY_DIR);
		File pubFile = new File(securityDir, PUBLIC_KEY_FILE);
		File privFile = new File(securityDir, PRIVATE_KEY_FILE);
	...
	/**
	 * Internal helper to write raw key bytes to disk in standard MIME
	 * Base64 PEM format.
	 */
