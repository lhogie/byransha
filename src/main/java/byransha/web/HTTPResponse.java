...
class HTTPResponse {
			...
			try {
				...
				if (supportsCbor && contentType.contains("application/json")) {
					try {
						...
						System.err.println("ERROR: Failed to convert JSON to CBOR: " + ex.getMessage());
						ex.printStackTrace();
				...
				boolean useGzip = acceptEncoding != null && acceptEncoding.contains("gzip")
						&& isCompressibleContentType(finalContentType) && responseData.length > 1024
						&& rangeStart == null;
				...
				if (useGzip) {
					...
					try (GZIPOutputStream gzipStream = new GZIPOutputStream(output, 8192);) {
					...
					if (rangeStart != null && rangeEnd != null) {
						...
						output.write(responseData, rangeStart.intValue(), rangeLength);
					} else {
						e.getResponseHeaders().set("Accept-Ranges", "bytes");
	...
	private boolean isCompressibleContentType(String contentType) {
		...
		return (lowerContentType.contains("text/") || lowerContentType.contains("application/json")
				|| lowerContentType.contains("application/javascript") || lowerContentType.contains("application/cbor")
				|| lowerContentType.contains("application/xml") || lowerContentType.contains("image/svg+xml"));
	}

	/**
	 * Determines if the content type represents a static resource that can be
	 * cached.
	 * 
	 * @param contentType The content type to check
	...
	private boolean isStaticResource(String contentType) {
		...
		String lowerContentType = contentType.toLowerCase();
		return (lowerContentType.contains("image/") || lowerContentType.contains("text/css")
				|| lowerContentType.contains("application/javascript") || lowerContentType.contains("font/")
				|| lowerContentType.contains("application/font") || lowerContentType.endsWith(".woff")
				|| lowerContentType.endsWith(".woff2") || lowerContentType.endsWith(".ttf"));
	}
}
