...
//			var g = new BBGraph(defaultDBDirectory);
	...
	public WebServer(BBGraph g, int port) throws Exception {
		...
		if (g.find(NodeInfo.class, endpoint -> true) == null)
			new NodeInfo(g);
		if (g.find(Views.class, endpoint -> true) == null)
			new Views(g);
		if (g.find(Jump.class, endpoint -> true) == null)
			new Jump(g);
		if (g.find(Endpoints.class, endpoint -> true) == null)
			new Endpoints(g);
		if (g.find(JVMNode.Kill.class, endpoint -> true) == null)
			new JVMNode.Kill(g);
		if (g.find(Authenticate.class, endpoint -> true) == null)
			new Authenticate(g);
		if (g.find(Nodes.class, endpoint -> true) == null)
			new Nodes(g);
		if (g.find(EndpointCallDistributionView.class, endpoint -> true) == null)
			new EndpointCallDistributionView(g);
		if (g.find(Info.class, endpoint -> true) == null)
			new Info(g);
		if (g.find(Logs.class, endpoint -> true) == null)
			new Logs(g);
		if (g.find(BasicView.class, endpoint -> true) == null)
			new BasicView(g);
		if (g.find(CharacterDistribution.class, endpoint -> true) == null)
			new CharacterDistribution(g);
		if (g.find(CharExampleXY.class, endpoint -> true) == null)
			new CharExampleXY(g);
		if (g.find(User.UserView.class, endpoint -> true) == null)
			new User.UserView(g);
		if (g.find(BBGraph.GraphNivoView.class, endpoint -> true) == null)
			new BBGraph.GraphNivoView(g);
		if (g.find(OSNode.View.class, endpoint -> true) == null)
			new OSNode.View(g);
		if (g.find(JVMNode.View.class, endpoint -> true) == null)
			new JVMNode.View(g);
		if (g.find(BNode.InOutsNivoView.class, endpoint -> true) == null)
			new BNode.InOutsNivoView(g);
		if (g.find(ModelGraphivzSVGView.class, endpoint -> true) == null)
			new ModelGraphivzSVGView(g);
		if (g.find(Navigator.class, endpoint -> true) == null)
			new Navigator(g);
		if (g.find(OutNodeDistribution.class, endpoint -> true) == null)
			new OutNodeDistribution(g);
		if (g.find(Picture.V.class, endpoint -> true) == null)
			new Picture.V(g);
//		if (g.find(AllViews.class, endpoint -> true) == null) new AllViews(g);
		if (g.find(LabView.class, endpoint -> true) == null)
			new LabView(g);
		if (g.find(ModelDOTView.class, endpoint -> true) == null)
			new ModelDOTView(g);
		if (g.find(SourceView.class, endpoint -> true) == null)
			new SourceView(g);
		if (g.find(ToStringView.class, endpoint -> true) == null)
			new ToStringView(g);
		if (g.find(StructureView.class, endpoint -> true) == null)
			new StructureView(g);
		if (g.find(NodeEndpoints.class, endpoint -> true) == null)
			new NodeEndpoints(g);
		if (g.find(SetValue.class, endpoint -> true) == null)
			new SetValue(g);
		if (g.find(AnyGraph.Classes.class, endpoint -> true) == null)
			new AnyGraph.Classes(g);
		if (g.find(Edit.class, endpoint -> true) == null)
			new Edit(g);
		if (g.find(IntrospectingEndpoint.class, endpoint -> true) == null)
			new IntrospectingEndpoint(g);
		if (g.find(History.class, endpoint -> true) == null)
			new History(g);
		new UI(g);
