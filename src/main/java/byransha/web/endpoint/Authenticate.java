...
public class Authenticate extends NodeEndpoint<BBGraph> {
	...
	public Authenticate(BBGraph db, int id) {
		...
		if (webServerInstance != null && webServerInstance.getSessionStore() != null) {
			...
			System.err.println("[ERROR] SessionStore not available during persisted endpoint loading for Authenticate ID: " + id);
	...
	public static void setSessionCookie(HttpsExchange https, String name, String value) {
		...
		String cookieValue = String.format("%s=%s; Path=/; Max-Age=%d; SameSite=None; Secure; HttpOnly",
	...
	public static void deleteSessionCookie(HttpsExchange https, String name) {
		String cookieValue = String.format("%s=; Path=/; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT; SameSite=None; Secure; HttpOnly", name);
		https.getResponseHeaders().add("Set-Cookie", cookieValue);
	}
	...
	@Override
	public EndpointJsonResponse exec(ObjectNode in, User _ignoredUserParameter, WebServer webServer, HttpsExchange https, BBGraph g)
			throws Throwable {
