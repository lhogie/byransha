...
public class ExportCSV extends NodeEndpoint<BNode> {
    ...
    public EndpointResponse exec(
        ...
        boolean includeAllFields =
            in.has("includeAllFields") &&
            in.get("includeAllFields").asBoolean(true);
        boolean includeAllNodes =
            in.has("includeAllNodes") &&
            in.get("includeAllNodes").asBoolean(true);
        ...
        if (includeAllNodes) {
            ...
            for (String className : includedNodeTypes) {
                try {
                    ...
                    Class<? extends BNode> nodeClass = (Class<
                        ? extends BNode
        ...
        try {
            ...
            if (node instanceof SearchForm) {
                ...
                if (nodesToExport.isEmpty()) {
                    return ErrorResponse.badRequest(
                        "No BusinessNodes found to export."
                    );
                }
            ...
            if (csvContent.isEmpty()) {
                return ErrorResponse.serverError(
                    "CSV export produced empty content."
                );
            }
            return new EndpointTextResponse("text/csv", pw ->
                pw.write(csvContent)
            );
        } catch (Exception e) {
            return ErrorResponse.serverError(
                "Error during CSV export: " + e.getMessage()
            );
        }
    }
}
