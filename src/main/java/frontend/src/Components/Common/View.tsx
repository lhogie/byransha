...
const MemoizedNetworkChart = memo(
	({
		...
		const _events = useMemo(() => ({ click: onChartClick }), [onChartClick]);
...
export const View = ({ viewId, sx }: { viewId: string; sx?: any }) => {
	...
	const _stringifyJson = useCallback(
	...
	const parseNivoChartData = useCallback((content: any) => {
		if (!content) return [];
	...
	const parseBarChartData = useCallback(
		(content: { [s: string]: { [s2: string]: string } }) => {
			if (!content) return {};
	...
	const getDistributionKeys = useCallback(
		(content: { [s: string]: any } | ArrayLike<unknown>) => {
			if (!content || Object.values(content).length === 0) return [];
			const keySet = new Set<string>();
			Object.values(content).forEach((obj: string) => {
	...
	const getNetworkData = useCallback((content: any) => {
		...
		// Process links
		const transformedLinks = content.links.map((link: any) => {
	...
	const displayContent = useCallback(
		(content: any, contentType: string) => {
			if (!content) {
