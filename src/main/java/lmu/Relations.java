...
public class Relations {
	...
	public static Set<InheritanceRelation> findInheritances(Set<Relation> relations) {
		var rr = relations.stream().filter(r -> r instanceof InheritanceRelation).toList();
		return new HashSet<InheritanceRelation>((Collection) rr);
	}
	...
	public static Set<AssociationRelation> convertAttributesToCompositions(Set<Attribute> attributes, Entity entity,
			Model model) {
	...
	public static Set<Entity> findSubClasses(Entity e, Set<Relation> relations) {
		...
		Set<InheritanceRelation> inheritanceRelations = Relations.findInheritances(relations);
